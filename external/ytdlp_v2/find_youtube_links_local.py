...
# 配置
YT_DLP = "/Users/luluqin766/Downloads/yt-dlp_macos2/yt-dlp_macosNightly"
CHANNEL_NAME = "DanKoeTalks"  # 可选：限制搜索范围到特定频道
...
def search_youtube_video(title, channel=None, max_results=5):
    ...
    try:
        ...
        if result.returncode != 0:
            print_colored(f"搜索失败: {result.stderr}", Colors.RED)
            return []
        ...
        for line in result.stdout.strip().split('\n'):
            ...
            try:
                ...
                # 记录错误但继续处理
                print_colored(f"解析视频信息时出错: {str(e)}", Colors.YELLOW)
                continue
        
        return videos
    
    except subprocess.TimeoutExpired:
        print_colored(f"搜索超时: {title}", Colors.YELLOW)
        return []
    except Exception as e:
...
def main():
    ...
    if args.csv_output is True:  # 如果只是设置了flag但没有值
        # 使用默认路径：与输入文件同目录下的 DanKoe_top30_summary.csv
        input_dir = Path(args.input_file).parent
        csv_output_file = str(input_dir / "DanKoe_top30_summary.csv")
